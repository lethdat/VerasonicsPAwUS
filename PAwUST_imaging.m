...
TX = repmat(struct('waveform', 1, ...
                     'Origin', [0.0 , 0.0 , 0.0] , ... 
                      ...
                      'Delay', zeros(1, Trans.numelements)), ... % plane wave
                            1, 2); 
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    Receive(2*i).Apod(1:Resource.Parameters.numRcvChannels) = 1.0;
...
for i = 1:Resource.RcvBuffer(2).numFrames
    % TX Receive left
    Receive(2*i-1 + nPaRcvs).Apod(1:Resource.Parameters.numRcvChannels) = 1.0;
    ...
    % TX Receive right
    Receive(2*i + nPaRcvs).Apod(1:Resource.Parameters.numRcvChannels) = 1.0;
    Receive(2*i + nPaRcvs).bufnum = 2;
    Receive(2*i + nPaRcvs).TGC = 2;
    Receive(2*i + nPaRcvs).framenum = i;
    Receive(2*i + nPaRcvs).acqNum = 2;
end
...
Process(3).Parameters = {'imgbufnum', 2, ... % number of buffer to process.
                         ...
                         'compressFactor', 40, ...
                         'mappingMethod', 'full', ...
                         'display', 2, ... % display image
                         'displayWindow', 2};

% - Storage
Process(4).classname ='Storage';
Process(4).method ='save';
...
for i = 1:Resource.RcvBuffer(1).numFrames
    ...
    %% Ultrasound frame
    Event(n).info ='US imaging for TX and Receive left';
    Event(n).tx = 2;
    ...
    Event(n).info ='US imaging for TX and Receive right';
    Event(n).tx = 2;
...
EF(1).Function = vsv.seq.function.ExFunctionDef('recon_worker', @recon_worker);
EF(2).Function = vsv.seq.function.ExFunctionDef('updatePaDisplayWindow', @updatePaDisplayWindow);
EF(3).Function = vsv.seq.function.ExFunctionDef('saveData', @saveData);

% UI control for changing reconstruction method
%UIButtonLabels{1} = {'SAFT', 'FSAFT', 'Fourier'};
UI(1).Control = vsv.seq.uicontrol.VsButtonGroupControl('LocationCode','UserB4', ...
                'Title', 'Recon Method', ...
                'NumButtons', 3, ...
                'Labels', {'SAFT', 'FSAFT', 'Fourier'}, ...
                'Callback',@chooseRecon);

% UI control for PA overlay
UI(2).Control = vsv.seq.uicontrol.VsToggleButtonControl('LocationCode','UserC2', ...
                'Label', 'PAT Overlay',...
                'Callback',@togglePatOverlay);

% UI control for changing the number of acquisitions to average over
UI(3).Control = vsv.seq.uicontrol.VsSliderControl('LocationCode','UserB2', ...
                'Label', '# Averages', ...
                'SliderMinMaxVal', [1, 10, 1], ...
                'SliderStep', [0.1, 1], ...
                'ValueFormat', '%2.0i',...
                'Callback',@numberOfAverages);


% - Save on/off button
UI(4).Control = vsv.seq.uicontrol.VsToggleButtonControl('LocationCode','UserB1',...
                        'Label','Save On/Off',...
                        'Callback',@SaveToggle);
...
function numberOfAverages(~,~,UIValue)
...
function togglePatOverlay(~,~) 
    persistent overlayImgHandle
    if UIState ~= 0
...
function ImgData = recon_worker(RData)
    persistent loopCnt RData_buf prevRData;
    ...
    RData_avg = int16(zeros(size(RData_buf{1})));
    avgCnt = 0;
...
function imgPData = updatePaDisplayWindow(imgData)
    persistent paImgHandle usImgHandle overlayImgHandle;
    ...
    if evalin('base', 'P.paOverlayToggled')
        ...
        if ~isempty(h) && ishandle(h)
            usImgHandle = evalin('base', 'Resource.DisplayWindow(2).imageHandle');
        end
    end
end
