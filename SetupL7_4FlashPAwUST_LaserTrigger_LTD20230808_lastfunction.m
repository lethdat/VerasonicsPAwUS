...
P(1).endDepth = 140;          % Acquisition end depth
P(2).startDepth = 0;          % Acquisition start depth in wavelengths
P(2).endDepth = 140;          % Acquisition end depth may be different for PA
% Set 2D parameters
...
ne = 2;         % ne = number of acquisitions in PA ensemble for coherent addition in I/Q buffer.
...
Resource.DisplayWindow(1).Colormap = grayscaleCFImap; %gray(256)
...
SeqControl(6).argument = 1000; % time in usec
% -- Jump back to start.
...
SeqControl(12).argument = 1; % noop counts are in 2 microsec increments fix(flash2Qdelay)*5
% sync command
SeqControl(13).command = 'sync';
SeqControl(13).argument  = 1e5; % in us
